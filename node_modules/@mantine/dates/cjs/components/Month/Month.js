...
const Month = React.forwardRef((props, ref) => {
  ...
  const weekdays = getWeekdaysNames.getWeekdaysNames(finalLocale, firstDayOfWeek, weekdayLabelFormat).map((weekday) => /* @__PURE__ */ React__default.createElement("th", {
    className: classes.weekdayCell,
    key: weekday
  }, /* @__PURE__ */ React__default.createElement(core.Text, {
    size,
    className: classes.weekday
  }, weekday.length >= 2 ? hooks.upperFirst(weekday) : weekday)));
  const hasValue = Array.isArray(value) ? value.every((item) => item instanceof Date) : value instanceof Date;
  const hasValueInMonthRange = value instanceof Date && dayjs__default(value).isAfter(dayjs__default(month).startOf("month")) && dayjs__default(value).isBefore(dayjs__default(month).endOf("month"));
