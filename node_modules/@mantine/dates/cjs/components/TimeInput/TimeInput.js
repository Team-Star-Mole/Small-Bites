...
const TimeInput = React.forwardRef((props, ref) => {
  ...
  const [time, setTime] = React.useState(getTimeValue.getTimeValues(value || defaultValue, format, amLabel, pmLabel));
  const [_value, setValue] = React.useState(value || defaultValue);
  hooks.useDidUpdate(() => {
    setTime(getTimeValue.getTimeValues(_value, format, amLabel, pmLabel));
  ...
  const setDate = (change) => {
    ...
    const newDate = getDate.getDate(timeWithChange.hours, timeWithChange.minutes, timeWithChange.seconds, format, pmLabel, timeWithChange.amPm);
    setValue(newDate);
